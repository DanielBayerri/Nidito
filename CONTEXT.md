...
- **Convertir una categoria en objectiu** (`convertCategoryToGoal`, botó 🏆 a la capçalera de la targeta, amb confirmació "Segur?"): pensat per a Viatges/Mascota, que en Daniel va decidir que havien de deixar de ser "pots" amb pressupost mensual propi i passar a rebre assignacions des del pot d'Objectius igual que Casament. **No esborra ni migra res automàticament**: `cat.archived = true` i `cat.budget` es congela a 0 (amb una entrada nova a `budgetHistory` perquè els mesos futurs no acumulin més, però els mesos passats mantenen el seu valor real; el pressupost mensual que tenia es desa a `cat.archivedMonthlyBudget`, només per als totals, mai per acumular), i es crea un objectiu nou amb el mateix nom/icona. Tot l'historial de despeses de la categoria arxivada es queda intacte (surt a "Tots els moviments", a l'exportació CSV i a "Gastat per categoria i mes" marcada "(arxivada)").
- **Les targetes de resum de dalt es van separar en dos parells (total + remanent)**, un per a les categories normals i un per a tot el que és Objectius (pot + categories fixades encara no convertides + saldo congelat de les ja convertides + el que ja tenen assignat els objectius concrets), en comptes d'una sola xifra barrejada:
  - `Pressupost conjunt` / `Disponible conjunt ara` → només categories normals (`mainCats`, ni fixades ni arxivades).
  - `Pressupost objectius` / `Estalviat en objectius` → pot d'Objectius + categories fixades (`goalCats`) + `archivedMonthlyBudget`/saldo de les arxivades + `goalSaved` de cada objectiu concret.
  - Es va treure la targeta "Ingressos totals" (no aportava res que no es veiés ja a "Diners personals").
